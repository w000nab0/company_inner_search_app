...
def display_select_mode():
    ...
    st.markdown(f"## {ct.SIDE_BAR_TITLE_1}")
    """
    回答モードのラジオボタンを表示
    """
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.warning(ct.ATTENTION)


def display_functional_explanation():
    """
    機能説明
    """
    with st.chat_message("assistant"):
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        main_page_number = None  # 初期化
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']} (ページNO.{main_page_number})", icon=icon)
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']} ({sub_choice['page_number']})", icon=icon)
                                else:
